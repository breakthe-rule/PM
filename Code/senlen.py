...
def senlen(maxlen,chars,target_chars,CX,CX_val,X,X_val,y_a,y_a_val,y_t,y_t_val,target_indices_char):
  # Define input layers
  cnn_input = Input(shape=(maxlen, len(chars) + 2))
  lstm_input = Input(shape=(maxlen, len(chars) + 6))
  ...
  mae = np.mean(np.abs(pred_time - y_t_val))
  print("MAE:", mae)
