...
def lstm(maxlen,chars,target_chars,CX,CX_val,X,X_val,y_a,y_a_val,y_t,y_t_val,target_indices_char,divisor):
  # Define input layers
  ...
  # start = time.time()
  # history = model.fit(train_data, train_labels, validation_data=(val_data,val_labels), verbose=1, 
  ...
  start = time.time()
  history = model.fit(train_data, train_labels, validation_data=(val_data,val_labels), verbose=1, 
                      callbacks=[ lr_reducer,early_stopping], epochs=100)
